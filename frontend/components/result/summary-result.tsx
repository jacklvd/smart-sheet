...
export default function SummaryResult({ result, originalText }: SummaryResultProps) {
    const [copied, setCopied] = useState(false);

    // Calculate reduction percentage if it doesn't exist in the result
    const reductionPercentage = result.reduction_percentage !== undefined
        ? result.reduction_percentage
        : result.original_length > 0
            ? ((result.original_length - result.summary_length) / result.original_length) * 100
            : 0;
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    ...
                    (from {result.original_length} to {result.summary_length} words)
                </CardDescription>
            </CardHeader>
            <CardContent>
